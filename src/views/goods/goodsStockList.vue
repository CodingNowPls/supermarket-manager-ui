<template>
  <div>
    ...
    <!--修改库存量-->
    <el-dialog title="修改商品库存量" :visible.sync="editVisible" width="60%">
      <el-form :model="editForm" :rules="rules" ref="editForm" label-width="150px" class="demo-ruleForm">
        <el-form-item style="width: 60%" label="商品名：">
          <el-input disabled v-model="editForm.name" placeholder="如：农夫山泉"></el-input>
        </el-form-item>

        <el-form-item label="基本库存量：" style="width: 60%" prop="inventory">
          <el-input type="number" @change="() => {
              if (editForm.inventory <= 0) {
                editForm.inventory = 1;
              }
            }
            " v-model="editForm.inventory" placeholder="如：10"></el-input>
        </el-form-item>
        <el-form-item label="货架基本数量：" style="width: 60%" prop="shelves">
          <el-input type="number" @change="() => {
              if (editForm.shelves <= 0) {
                editForm.shelves = 1;
              }
            }
            " v-model="editForm.shelves" placeholder="如：10"></el-input>
        </el-form-item>
        <el-form-item style="width: 60%">
          ...
          <el-button @click="closeEdit('editForm')" style="font-size: 18px">关闭
          </el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
