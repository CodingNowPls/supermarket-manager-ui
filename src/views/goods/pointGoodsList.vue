  ...
  <div>
    ...
    <el-row>
      <el-col :span="24" style="text-align: left; padding-right: 10px">
        ...
        <el-button type="success" @click="newBtn" style="font-size: 18px">
    ...
    <div class="table">
      <el-table :data="tableData" style="width: 100%" size="medium">
        ...
        <el-table-column prop="integral" :show-overflow-tooltip="true" label="积分">
        </el-table-column>
        <el-table-column prop="updateby" :show-overflow-tooltip="true" label="操作者">
        </el-table-column>
        <el-table-column width="240" fixed="right" label="操作">
          <template v-slot="scope">
            <el-button type="success" @click="editBtn(scope.row.goodsId)" style="font-size: 18px">
    ...
    <el-dialog title="创建积分商品" :visible.sync="newVisable" width="50%">
      <el-form :model="newForm" :rules="rules" ref="newForm" class="demo-ruleForm">
        <el-form-item v-if="selectGoodsVisiable" label="商品" prop="goodsId">
          <el-select v-model="newForm.goodsId" placeholder="请选择商品" filterable @change="$forceUpdate()" clearable>
            <el-option v-for="item in options_goods" :key="item.id" :label="item.name" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item v-else label="商品" prop="goodsId">
          <el-select disabled v-model="newForm.goodsId" placeholder="请选择商品" filterable @change="$forceUpdate()"
            clearable>
            <el-option v-for="item in options_goods" :key="item.id" :label="item.name" :value="item.id">
            </el-option>
          </el-select>
        ...
        <el-form-item v-if="!selectGoodsVisiable" label="图片：">
          <img style="width: 100px" :src="BaseApi + newForm.coverUrl" alt="商品图片"/>
        </el-form-item>
        <el-form-item v-if="!selectGoodsVisiable" label="积分：" prop="integral">
          <el-input @change="() => {
        ...
        <el-form-item v-if="selectGoodsVisiable">
          <el-button type="primary" @click="confirmGoods('newForm')" style="font-size: 18px">
            <i class="iconfont icon-r-yes" style="font-size: 18px">
            </i>
            确定
          </el-button>
          <el-button @click="closeGoods('newForm')" style="font-size: 18px">关闭
          </el-button>
        </el-form-item>
        <el-form-item v-else>
          ...
          <el-button @click="cancel('newForm')" style="font-size: 18px">取消
          </el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    ...
    <el-dialog title="修改积分商品" :visible.sync="editVisable" width="50%">
      <el-form :model="editForm" :rules="rules" ref="editForm" class="demo-ruleForm">
        <el-form-item label="商品" prop="goodsId">
          <el-select disabled v-model="editForm.goodsId" placeholder="请选择商品" filterable @change="$forceUpdate()"
            clearable>
            <el-option v-for="item in options_goods1" :key="item.id" :label="item.name" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="图片：">
          <img style="width: 100px" :src="BaseApi + editForm.coverUrl" alt="商品图片"/>
        </el-form-item>
        <el-form-item label="积分：" prop="integral">
          <el-input @change="() => {
            ...
            " type="number" v-model="editForm.integral" placeholder="如：3000"/>
        </el-form-item>
        <el-form-item>
          ...
          <el-button @click="cancelEdit('editForm')" style="font-size: 18px">取消
          </el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
